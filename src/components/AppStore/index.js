...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredApps = this.getActiveTabApp(searchResults)

    return (
      <div className="app-container">
        <div className="aap-store-container">
          ...
          <ul className="tabs-list">
            {tabsList.map(eachTabs => (
              <TabItem
                key={eachTabs.tabId}
                tabItemDetails={eachTabs}
                setActiveTabId={this.setActiveTabId}
                isActive={activeTabId === eachTabs.tabId}
              />
            ))}
          </ul>

          <ul className="apps-list">
            {filteredApps.map(eachApps => (
              <AppItem key={eachApps.appId} appItemDetails={eachApps} />
            ))}
          </ul>
        </div>
      </div>
